...
public class ConexionWS {
    ...
    public static RespuestaHTTP peticionDELETEE(String url) {
    RespuestaHTTP respuesta = new RespuestaHTTP();
    try {
        URL urlDestino = new URL(url);
        HttpURLConnection conexionHttp = (HttpURLConnection) urlDestino.openConnection();
        ...
        if (codigoRespuesta == HttpURLConnection.HTTP_OK) {
            respuesta.setContenido(obtenerContenidoWS(conexionHttp.getInputStream()));
        } else if (codigoRespuesta == HttpURLConnection.HTTP_FORBIDDEN) {
            // Ignorar contenido para el error 403
            respuesta.setContenido("Acceso prohibido (HTTP 403)"); // Mensaje simple en lugar del HTML completo
        } else {
            InputStream errorStream = conexionHttp.getErrorStream();
            if (errorStream != null) {
                respuesta.setContenido(obtenerContenidoWS(errorStream));
            } else {
                respuesta.setContenido("Código de respuesta HTTP: " + codigoRespuesta);
            }
        }
